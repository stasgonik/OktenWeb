const { statusCodeEnum: statusCode } = require('../constant');
const { ErrorHandler, passwordHasher } = require('../helper');
const { errorMessage } = require('../message');
const { userService } = require('../service');
const { userValidators, utilValidators } = require('../validator');

module.exports = {
    isUserValid: async (req, res, next) => {
        try {
            const { error } = await userValidators.createUserValidator.validate(req.body);

            if (error) {
                throw new ErrorHandler(statusCode.BAD_REQUEST,
                    errorMessage.JOI_VALIDATION_ERROR.customCode,
                    error.details[0].message);
            }

            next();
        } catch (e) {
            next(e);
        }
    },

    isSearchQueryValid: async (req, res, next) => {
        try {
            const filter = req.query;

            delete filter.preferL;

            if (!filter) {
                return next();
            }

            const { error } = await userValidators.findUserValidator.validate(filter);

            if (error) {
                throw new ErrorHandler(statusCode.BAD_REQUEST,
                    errorMessage.JOI_VALIDATION_ERROR.customCode,
                    error.details[0].message);
            }

            next();
        } catch (e) {
            next(e);
        }
    },

    isUserAlreadyExist: async (req, res, next) => {
        try {
            const { body: { email }, query: { preferL = 'en' } } = req;

            const user = await userService.findOneUser({ email });

            if (user) {
                throw new ErrorHandler(statusCode.BAD_REQUEST,
                    errorMessage.USER_ALREADY_EXIST.customCode,
                    errorMessage.USER_ALREADY_EXIST[preferL]);
            }

            next();
        } catch (e) {
            next(e);
        }
    },

    isUpdateUserValid: async (req, res, next) => {
        try {
            const { error } = await userValidators.updateUserValidator.validate(req.body);

            if (error) {
                throw new ErrorHandler(statusCode.BAD_REQUEST,
                    errorMessage.JOI_VALIDATION_ERROR.customCode,
                    error.details[0].message);
            }

            next();
        } catch (e) {
            next(e);
        }
    },

    isAccountActivated: (req, res, next) => {
        try {
            const { query: { preferL = 'en' }, user } = req;

            if (!user.activation_status) {
                throw new ErrorHandler(statusCode.UNAUTHORIZED,
                    errorMessage.ACCOUNT_IS_NOT_ACTIVATED.customCode,
                    errorMessage.ACCOUNT_IS_NOT_ACTIVATED[preferL]);
            }

            next();
        } catch (e) {
            next(e);
        }
    },

    isForgotPasswordValid: async (req, res, next) => {
        try {
            const { email, old_password } = req.body;

            const { error } = await utilValidators.forgotPasswordValidator.validate(req.body);

            if (error) {
                throw new ErrorHandler(statusCode.BAD_REQUEST,
                    errorMessage.JOI_VALIDATION_ERROR.customCode,
                    error.details[0].message);
            }

            const user = await userService.findOneUser({ email }, 'getAll');

            if (!user) {
                throw new ErrorHandler(statusCode.NOT_FOUND,
                    errorMessage.NO_RESULT_FOUND.customCode,
                    errorMessage.NO_RESULT_FOUND.en);
            }

            await passwordHasher.compare(old_password, user.password);

            req.user = user;

            next();
        } catch (e) {
            next(e);
        }
    },

    accessRightsCheck: (req, res, next) => {
        try {
            const { params: { userId }, query: { preferL = 'en' }, user } = req;

            if (+userId !== user.id) {
                throw new ErrorHandler(statusCode.UNAUTHORIZED,
                    errorMessage.UNAUTHORISED_ACCESS.customCode,
                    errorMessage.UNAUTHORISED_ACCESS[preferL]);
            }

            next();
        } catch (e) {
            next(e);
        }
    },
};

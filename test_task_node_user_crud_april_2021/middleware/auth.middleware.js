...
const {
    config: {
        JWT_SECRET, JWT_REFRESH, JWT_ACTIVATION
    }
} = require('../config');
...
module.exports = {
    ...
    checkActivationToken: async (req, res, next) => {
        try {
            const { preferL = 'en', activation_token } = req.query;

            if (!activation_token) {
                throw new ErrorHandler(statusCode.BAD_REQUEST,
                    errorMessage.ABSENT_ACTIVATION_TOKEN.customCode,
                    errorMessage.ABSENT_ACTIVATION_TOKEN[preferL]);
            }

            jwt.verify(activation_token, JWT_ACTIVATION, (err) => {
                if (err) {
                    throw new ErrorHandler(statusCode.UNAUTHORIZED,
                        errorMessage.ACTIVATION_TOKEN_NOT_VALID.customCode,
                        errorMessage.ACTIVATION_TOKEN_NOT_VALID[preferL]);
                }
            });

            const user = await userService.findOneUser({ activation_token }, 'getAll');
